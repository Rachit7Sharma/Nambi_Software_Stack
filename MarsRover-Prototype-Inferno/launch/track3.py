...
# Initialize ROS node and publisher for move_base_simple goal
...
while(True):
    ...
    # Use OpenCV to detect the arrow in the frame
    # ...
    # ...
    found, theta, orient, direction, img = arrow_detect(frame)

    if found:
        # Get the pose of the detected arrow
        # ...
        # ...
        


        # Create a goal message with the arrow's pose
        goal = PoseStamped()
        goal.header.frame_id = "camera_frame"
        goal.pose.position.x = arrow_pose[0]
        goal.pose.position.y = arrow_pose[1]
